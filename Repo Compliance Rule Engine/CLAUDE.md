...
- Child rules aggregate by dimension and check whether the net falls within `[lower_bound, upper_bound]`:
  - `counterparty` — net notional by counterparty
  - `maturity_month` — net notional by maturity month
  - `counterparty_mtm` — aggregate the `mtm` field (not notional) by counterparty; bounds are absolute MtM limits
  - `counterparty_pct` — net notional by counterparty expressed as a percentage of the rule's `total_limit`; bounds are percentages
